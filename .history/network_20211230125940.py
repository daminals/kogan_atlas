...
class Block:
    ...
    def hash_block(self):
        protected_data = json.dumps(self.data_dict())
        block_encryption = hashlib.sha256(protected_data.encode())
        return block_encryption.hexdigest()
    
    def data_dict(self):
        return {'index': str(self.index), 'timestamp': str(self.timestamp), 'data': str(self.data), 'prev_hash': str(self.prev_hash), 'nonce': str(self.nonce), 'hash': self.hash}
