...
class Blockchain:
    ...
    def mine(self):
        ...
        new_block = Block(last_block.index + 1, datetime.now(), self.unconfirmed_data, last_block.prev_hash)
        proof = self.proof_of_work(new_block)
        self.add_block(new_block, proof)
        self.unconfirmed_data = self.unconfirmed_data[:]
...
class Block:
    ...
    def data_dict(self):
        return {'id': str(self.index), 'timestamp': str(self.timestamp), 'hash':str(self.hash), 'prev_hash': str(self.prev_hash)}
